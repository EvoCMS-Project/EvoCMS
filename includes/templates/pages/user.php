<div class="row">
		...
		<?php foreach(Evo\Social::getProviders() as $network => [$name, $icon]) { ?>
			<?php if (!empty($user_info->social[$network])) { ?>
				<a target="_blank" href="<?= $user_info->social[$network] ?>">
					<span class="fa-solid fa-stack fa-lg" title="<?= $name ?>">
						<i class="fa-solid fa-square fa-stack-2x"></i>
						<i class="fa-brands <?= $icon ?> fa-stack-1x fa-inverse"></i>
					</span>
	...
	<div class="col-sm-5 col-3 text-end">
		...
		if (!$is_mine) {
			echo '<button class="btn btn-danger" title="' . __('user.report') . '" onclick="report(' . $user_info->id . ');"><i class="fa-solid fa-flag fa-2x"></i></button> ';
		}

		if (has_permission('mod.ban_member')) {
			echo '<a class="btn btn-danger" title="' . __('user.ban') . '" href="' . App::getAdminURL('banlist', ['username' => $user_info->username]).'"><i class="fa-solid fa-ban fa-2x"></i></a> ';
		}

		if ($can_mod) {
			echo '<a class="btn btn-warning" title="' . __('user.manage') . '" href="' . App::getAdminURL('users', ['filter' => 'username:'.$user_info->username]) . '"><i class="fa-solid fa-user-secret fa-2x"></i></a> ';
		}
		?>
	</div>
</div>
...
<div class="row text-center">
    ...
    <div class="col">
        <a title="<?= __('user.comments') ?>"><strong><?= $num_comments ?></strong> <i class="fa-solid fa-comment"></i></a>
    </div>
    <div class="col">
        <a title="<?= __('user.friends') ?>"><strong><?= $num_friends ?></strong> <i class="fa-solid fa-user"></i></a>
    </div>
    <div class="col">
...
<?php if (!$is_mine): ?>
	...
	<div class="row">
		<div class="col-md-6">
			<a class="btn btn-success btn-sm w-100" href="<?= App::getURL('mail/'.$user_info->username) ?>#mail"><i class="fa-solid fa-envelope"></i> <?= __('user.send_message') ?></a>
		 </div>
		 <div class="col-md-6">
			<form method="post" action="<?= App::getURL('friends') ?>">
				<button class="btn btn-warning btn-sm w-100" name="new_friend" value="<?= $user_info->username ?>"><i class="fa-solid fa-user"></i> <?= __('user.add_friend') ?></button>
			</form>
		 </div>
	</div>
<?php endif; ?>
...
<div class="user-profile-content">
    ...
    <div class="row">
        <div class="col-sm-6">
			<h6><strong><i class="fa-solid fa-earth-americas"></i> <?= __('user.country') ?></strong></h6>
