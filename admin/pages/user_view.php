...
$tab = App::GET('tab', 'user');

if (IS_POST && App::POST('admin_user_view_tab')) {
	$tab = App::POST('admin_user_view_tab');
}

if (IS_POST && $tab === 'profile' && has_permission('admin.edit_uprofile')) {
	$user_info = profile_process_user_update($user_info, true);
}
...
<div class="admin-dashboard admin-user-view">
	...
	<section class="admin-tabs-board admin-user-view-board">
		<?= admin_user_view_nav($user_tabs, $tab) ?>
		...
		<div class="tab-content admin-tabs-board__body admin-user-view-board__body admin-tabs-panel admin-user-view-board__body--content">
			<?= admin_user_view_tab_open('user', $tab === 'user') ?>
				<?= admin_user_view_profile_board($user_info) ?>
			<?= admin_user_view_tab_close() ?>

			<?= admin_user_view_tab_open('profile', $tab === 'profile') ?>
				<?php if (has_permission('admin.edit_uprofile')): ?>
					<?= admin_user_view_edit_board($user_info) ?>
				<?php else: ?>
