...
$comment_status = [0 => 'Ok',  1=> 'Ok', 2 => 'Censuré'];
...
if (!$comments) {
	return print '<div id="content"><div style="text-align: center;" class="alert alert-warning">Aucun commentaire!</div></div>';
}
?>
				...
				<?php
					foreach($comments as $comment) {
						...
						echo '<tr>';
							...
							echo '<td>';
								if ($comment['state'] == 2)
									echo '<button class="btn btn-sm btn-success" name="com_accept" value="'.$comment['id'].'" title="Autoriser ce commentaire"><i class="fa fa-check"></i></button> ';
								if (has_permission('mod.comment_censure') && $comment['state'] != 2)
									echo '<button name="com_censure" value="'.$comment['id'].'" title="Censurer le contenu" class="btn btn-sm btn-danger"><i class="fa fa-ban"></i></button> ';
								if (has_permission('mod.comment_delete'))
									echo '<button name="com_delete" value="'.$comment['id'].'" title="Supprimer" class="btn btn-sm btn-danger"><i class="far fa-trash-alt"></i></button> ';
								echo '<a href="'.App::getURL($comment['page_id'], [], '#msg'.$comment['id']).'" title="Voir la page" class="btn btn-sm btn-primary"><i class="fa fa-eye"></i></a>';
							echo '</td>';
						echo '</tr>';
					}
